...
import controlador.CTipoProducto;
import modelo.MTipoProductos;

public class tipoProducto extends JDialog {
	...
	private CTipoProducto controladorTipo;
	private int idSeleccionado = -1;
	private JButton btnAtras;
	
	public static void main(String[] args) {
		try {
			tipoProducto dialog = new tipoProducto(idioma);
	...
	public tipoProducto(Locale idioma) {
		setIconImage(Toolkit.getDefaultToolkit().getImage(tipoProducto.class.getResource("/imagenes/v914-ning-21a.png")));
		// Internacionalización
		tipoProducto.idioma = idioma;
		...
		{
			lblTipo = new JLabel(et.getString("tipoproductos"));
			lblTipo.setFont(new Font("Bahnschrift", Font.BOLD, 15));
			lblTipo.setHorizontalAlignment(SwingConstants.CENTER);
			lblTipo.setBounds(20, 45, 200, 20);
			contentPanel.add(lblTipo);
		}
		{
			txtFldTipo = new JTextField();
			txtFldTipo.setBounds(20, 65, 200, 20);
			contentPanel.add(txtFldTipo);
			txtFldTipo.setColumns(10);
			txtFldTipo.setEnabled(false);
		}
		{
		...
		}
		{
			...
			modeloTabla.setColumnIdentifiers(
					new String[] { et.getString("codigoproductos"), et.getString("tipoproductos")});
		}
		{
			tablaTipo = new JTable();
			tablaTipo.setModel(modeloTabla);
			scrollPane.setViewportView(tablaTipo);
		}
		{
		...
		}
		{
			...
			btnNuevo.setIcon(new ImageIcon(tipoProducto.class.getResource("/imagenes/anadir.png")));
		...
		}
		{
			...
			btnGuardar.setIcon(new ImageIcon(tipoProducto.class.getResource("/imagenes/salvar.png")));
		...
		}
		{
			...
			btnActualizar.setIcon(new ImageIcon(tipoProducto.class.getResource("/imagenes/actualizar-flecha.png")));
		...
		}
		{
			...
			btnEliminar.setIcon(new ImageIcon(tipoProducto.class.getResource("/imagenes/borrar (1).png")));
		...
		}
		{
			...
			btnLimpiar.setIcon(new ImageIcon(tipoProducto.class.getResource("/imagenes/escoba.png")));
		...
		}
		{
			...
			btnModificar.setIcon(new ImageIcon(tipoProducto.class.getResource("/imagenes/boligrafo-cuadrado.png")));
		...
		}
		{
			...
			btnAtras.setIcon(new ImageIcon(tipoProducto.class.getResource("/imagenes/angulo-circulo-izquierda.png")));
		...
		controladorTipo = new CTipoProducto();
		mostrarTabla();
	}
	...
	public class ManejadorBotones implements ActionListener {
		public void actionPerformed(ActionEvent evento) {
			if (evento.getSource().equals(btnAtras)) {
				// Regresar al menu
				gestionproductos tipoProducto = new gestionproductos(idioma);
	...
	private void guardar() {
		// Guardar datos en bd
		String tipo = txtFldTipo.getText();

		MTipoProductos tipos = new MTipoProductos(tipo);
		controladorTipo.anadirTipo(tipos);
	}
	
	private void seleccionarID() {
		int filaSeleccionada = tablaTipo.getSelectedRow();
		if (filaSeleccionada >= 0) {
			idSeleccionado = (int) modeloTabla.getValueAt(filaSeleccionada, 0);
			txtFldTipo.setText((String) modeloTabla.getValueAt(filaSeleccionada, 1));
		} else {
	...
	private void modificar() {
		if (idSeleccionado != -1) {
			...
			controladorTipo.actualizarTipo(tipo, idSeleccionado);
		} else {
	...
	private void eliminar() {
		int filaSeleccionada = tablaTipo.getSelectedRow();
		int idtipoProductos = (int) modeloTabla.getValueAt(filaSeleccionada, 0);
		controladorTipo.eliminarTipo(idtipoProductos);
	}
	...
	private void mostrarTabla() {
		...
		List<MTipoProductos> listaTipos = controladorTipo.obtenerTipo();

		// Añadir clientes a la tabla
		for (MTipoProductos tipos : listaTipos) {
			modeloTabla.addRow(new Object[] { tipos.getIdtipoProductos(), tipos.getTipo()});
		}
	}
}
