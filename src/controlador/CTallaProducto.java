...
public class CTallaProducto {
	...
	public void anadirTalla(MTallaProducto talla) {
		...
		try {
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "Error al añadir la talla", "Error", JOptionPane.ERROR_MESSAGE);
		}
	}

	public void eliminarTalla(int idtallaProductos) {
		...
		sql = "DELETE FROM tallaProducto WHERE idtallaProd = ?";

		try {
			sentencia = Conexion.prepareStatement(sql);
			sentencia.setInt(1, idtallaProductos);
			...
			System.err.println("Error de SQL: " + e.getMessage());
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "Error al eliminar la talla", "Error", JOptionPane.ERROR_MESSAGE);
		}
	}

	public void actualizarTalla(String talla, int idtallaProductos) {
		...
		sql = "UPDATE tallaProducto SET talla = ? WHERE idtallaProd = ?";

		try {
			Conexion = ConexionBDSQLServer.GetConexion();
			sentencia = Conexion.prepareStatement(sql);
			sentencia.setString(1, talla);
			sentencia.setInt(2, idtallaProductos);
			...
			System.err.println("Error de SQL: " + e.getMessage());
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "Error al actualizar la talla.", "Error", JOptionPane.ERROR_MESSAGE);
		} finally {
			try {
				if (sentencia != null)
					sentencia.close();
	...
	public List<MTallaProducto> obtenerTalla() {
		List<MTallaProducto> listaTallas = new ArrayList<>();
		Connection conexion = ConexionBDSQLServer.GetConexion();
		String sql = "SELECT * FROM tallaProducto";

		try (PreparedStatement PS = conexion.prepareStatement(sql); ResultSet RS = PS.executeQuery()) {
			while (RS.next()) {
				// Extraer datos de cada fila
				int idtallaProductos = RS.getInt("idtallaProd");
				String talla = RS.getString("talla");

				// Crear objeto MClientes y añadirlo a la lista
				MTallaProducto tallas = new MTallaProducto(idtallaProductos, talla);
				listaTallas.add(tallas);
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
